...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // ===============================
    // Personalize from stored session
    // (set by auth.js at login/signup)
    // ===============================

    try{

        const stored=JSON.parse(localStorage.getItem("ai_helpdesk_user") || "null");
        ...
        if(stored && stored.name){
            ...
            const firstName=stored.name.trim().split(" ")[0];
            const initials=stored.name.trim().split(" ")
                .map(part=>part[0])
                .join("")
                .slice(0,2)
                .toUpperCase();
            ...
            if(userName) userName.textContent=stored.name;
            if(userAvatar && initials) userAvatar.textContent=initials;

        }

    }catch(err){

        // Corrupt or missing session data — keep the default placeholders.
...
if (analyzeSkillBtn) {
    ...
    analyzeSkillBtn.addEventListener("click", () => {
        ...
        //     body: JSON.stringify({ role, skills: currentSkillsArr })
        // })
        // .then(res => res.json())
        // .then(data => applySkillGapAnalysis(data))
        // .catch(() => showDashboardToast("Skill gap analysis failed. Please try again.", "error"))
        // .finally(() => {
        ...
        setTimeout(() => {
            ...
            window.logActivity?.(`Skill gap analyzed for ${role} — ${result.score}% ready.`, "fa-chart-line");
...
if (roadmapBtn) {
    ...
    roadmapBtn.addEventListener("click", () => {
        ...
        // ===============================
        //
        // fetch("/api/learning-planner", {
        //     method: "POST",
        //     headers: { "Content-Type": "application/json" },
        //     body: JSON.stringify({ role: selectedRole })
        // })
        // .then(res => res.json())
        // .then(data => applyLearningPlan(data))
        // .catch(() => showDashboardToast("Couldn't generate a plan. Please try again.", "error"))
        // .finally(() => {
        ...
        setTimeout(() => {
            ...
            showDashboardToast(`Learning plan generated for ${selectedRole}.`, "success");
...
if (analyzeResumeBtn) {
    ...
    const analyzeDefaultLabel = analyzeResumeBtn.textContent;

    analyzeResumeBtn.addEventListener("click", () => {

        if (!currentResumeFile) {

            showDashboardToast("Please upload your resume first.", "error");
            return;

        }

        const role = document.getElementById("resumeRole").value;
        ...
        // })
        // .then(res => res.json())
        // .then(data => applyResumeAnalysis(role, data))
        // .catch(() => showDashboardToast("Analysis failed. Please try again.", "error"))
        // .finally(() => {
        //     analyzeResumeBtn.disabled = false;
        //     analyzeResumeBtn.textContent = analyzeDefaultLabel;
        // });

        setTimeout(() => {

            const data = resumeRoleProfiles[role] || resumeRoleProfiles["AI Engineer"];

            applyResumeAnalysis(role, data);

            analyzeResumeBtn.disabled = false;
            analyzeResumeBtn.textContent = analyzeDefaultLabel;

            showDashboardToast(`Resume analyzed for ${role}.`, "success");
            window.logActivity?.(`Resume analyzed for ${role} — scored ${data.score}%.`, "fa-file-circle-check");
...
if (generateMentorReportBtn) {
    ...
    generateMentorReportBtn.addEventListener("click", () => {
        ...
        // ===============================
        //
        // fetch("/api/mentor-report", {
        //     method: "POST",
        //     headers: { "Content-Type": "application/json" },
        //     body: JSON.stringify(getMentorInputs())
        // })
        // .then(res => res.json())
...
if (saveMemoryBtn) {
    ...
    saveMemoryBtn.addEventListener("click", () => {
        ...
        setTimeout(() => {
            ...
            // Lets the LangGraph Orchestrator (Phase 12) detect that this
            // agent has been used, without changing anything else here.
...
if (generateProjectsBtn) {
    ...
    generateProjectsBtn.addEventListener("click", () => {
        ...
        //     headers: { "Content-Type": "application/json" },
        //     body: JSON.stringify({
        //         skills, career_goal: goal,
        //         interests: projectInterests.value.trim(),
        //         domain, experience_level: experience
        //     })
        // })
        // .then(res => res.json())
        // .then(data => renderProjectOptions(data.recommendations, domain, experience))
        // .catch(() => showDashboardToast("Couldn't generate recommendations. Please try again.", "error"))
        // .finally(() => {
        ...
        setTimeout(() => {
            ...
            showDashboardToast(`3 project ideas generated for ${domain}.`, "success");
...
const generateInternship = document.getElementById("generateInternship");
...
if (generateInternship) {

    const generateDefaultLabel = generateInternship.innerHTML;

    generateInternship.addEventListener("click", () => {

        const skills = internshipSkills.value.trim();
        const cgpa = parseFloat(internshipCgpa.value);

        if (!skills || !internshipCgpa.value || !internshipRole.value) {

            showDashboardToast("Please enter your Skills, CGPA, and Preferred Role.", "error");
            return;

        }

        generateInternship.disabled = true;
        generateInternship.innerHTML =
            '<i class="fa-solid fa-spinner fa-spin" aria-hidden="true"></i> Generating...';
        ...
        //
        // const formData = new FormData();
        // if (currentInternshipResumeFile) formData.append("resume", currentInternshipResumeFile);
        // formData.append("skills", skills);
        // formData.append("cgpa", cgpa);
        // formData.append("role", internshipRole.value);
        // formData.append("company", internshipCompany.value);
        // formData.append("domain", internshipDomain.value);
        // formData.append("location", internshipLocation.value);
        // formData.append("stipend", internshipStipend.value);
        // formData.append("availability", internshipAvailability.value);
        //
        // fetch("/api/internship-recommendation", { method: "POST", body: formData })
        // .then(res => res.json())
        // .then(data => renderInternshipResults(data.recommendations, data.preferences))
        // .catch(() => showDashboardToast("Couldn't generate recommendations. Please try again.", "error"))
        // .finally(() => {
        //     generateInternship.disabled = false;
        //     generateInternship.innerHTML = generateDefaultLabel;
        // });
        ...
        setTimeout(() => {
            ...
            const prefs = {
                role: internshipRole.value,
                company: internshipCompany.value,
                domain: internshipDomain.value,
                location: internshipLocation.value.trim(),
                stipend: parseFloat(internshipStipend.value) || 0,
                availability: internshipAvailability.value,
                cgpa

            };

            const scored = internshipPool
                .map(i => scoreInternship(i, userSkillsLower, prefs))
                .sort((a, b) => b.score - a.score)
                .slice(0, 3);

            renderInternshipResults(scored, prefs);

            generateInternship.disabled = false;
            generateInternship.innerHTML = generateDefaultLabel;

            showDashboardToast(`3 internship matches found for ${prefs.role}.`, "success");
            window.logActivity?.(`Internship recommendations generated for ${prefs.role}.`, "fa-briefcase");
